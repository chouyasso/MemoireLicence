...
<body>
    <div class="btn">
        <button class="btn_con" onclick="navigateTo('vue_les_reponce_con.php', 'consultation')">Reponse conn</button>
        <button class="btn_vac" onclick="navigateTo('vue_les_reponce_Vac.php', 'vaccination')">Reponse vacc</button>
    </div>

     <h2>les reponse de Vaccinations</h2>
    ...
    <table>
         <tr>
            ...
            <th>le rendez vous fix</th>
            <th>Vaccin disponible</th>
            <th>Prix</th>
        ...
        if (!empty($search)) {
            $sql = "SELECT * FROM rep_vac WHERE  vet_id LIKE '%$search%' ORDER BY nouvaeu_date ASC";
        } else {
            $sql = "SELECT * FROM rep_vac  ORDER BY nouvaeu_date ASC";
        }
        ...
        if (mysqli_num_rows($result) == 0) {
            echo '<tr><td colspan="9" class="no-records">لا توجد سجلات لعرضها.</td></tr>';
        } else {
            while ($row = mysqli_fetch_assoc($result)): ?>
                <tr>
                    ...
                    <td>
                        <a href="modifier_vac.php?ID=<?php echo htmlspecialchars($row['ID_rep']); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    window.navigateTo = function(url, type) {
        if (confirm(`Êtes-vous sûr de vouloir afficher l’historique de   ${type} ?`)) {
